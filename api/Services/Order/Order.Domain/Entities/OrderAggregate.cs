...
public class OrderAggregate : AggregateRoot
{
    ...
    public static Result<OrderAggregate> Create(Guid customerId, Address shippingAddress, string? notes = null)
    {
    ...
    /// <summary>
    ///     Adds an item using product snapshot data (no direct Product entity reference).
    ///     Stock is managed by Catalog Service via Kafka events.
    /// </summary>
    public Result AddItem(Guid productId, string productName, Money unitPrice, int quantity)
    {
        ...
        if (_items.Count > 0 && _items.First().UnitPrice.Currency != unitPrice.Currency)
        {
        ...
        {
            var increaseResult = existingItem.IncreaseQuantity(quantity, unitPrice);
