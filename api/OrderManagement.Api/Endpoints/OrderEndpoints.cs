...
public static class OrderEndpoints
{
    public static IEndpointRouteBuilder MapOrderEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("", PlaceOrderAsync).WithName("PlaceOrder").WithSummary("Place a new order");
        group.MapGet("", GetAllOrdersAsync).WithName("GetAllOrders").WithSummary("Get all orders");
        group.MapGet("/{id:guid}", GetOrderAsync).WithName("GetOrder").WithSummary("Get order by ID");
        group.MapGet("/customer/{customerId:guid}", GetCustomerOrdersAsync).WithName("GetCustomerOrders")
            .WithSummary("List orders for a customer");
        group.MapPost("/{id:guid}/confirm", ConfirmOrderAsync).WithName("ConfirmOrder")
            .WithSummary("Confirm a pending order").RequireAuthorization("order:confirm");
        group.MapPost("/{id:guid}/ship", ShipOrderAsync).WithName("ShipOrder").WithSummary("Mark order as shipped")
            .RequireAuthorization("order:ship");
        group.MapPost("/{id:guid}/cancel", CancelOrderAsync).WithName("CancelOrder").WithSummary("Cancel an order");
        group.MapPost("/{id:guid}/deliver", DeliverOrderAsync).WithName("DeliverOrder")
            .WithSummary("Mark order as delivered").RequireAuthorization("order:deliver");
        group.MapDelete("/{id:guid}", DeleteOrderAsync).WithName("DeleteOrder")
    ...
    private static async Task<IResult> PlaceOrderAsync(
        PlaceOrderRequest request, IValidator<PlaceOrderRequest> validator,
        OrderService svc, HttpContext httpContext, CancellationToken ct)
    {
    ...
    private static async Task<IResult> GetAllOrdersAsync(
        OrderService svc, int page = 1, int pageSize = 100, CancellationToken ct = default)
    {
    ...
    private static async Task<IResult> GetCustomerOrdersAsync(
        Guid customerId, OrderService svc, int page = 1, int pageSize = 20, CancellationToken ct = default)
    {
    ...
    private static async Task<IResult> ConfirmOrderAsync(
        Guid id, OrderService svc, HttpContext httpContext, CancellationToken ct)
    {
    ...
    private static async Task<IResult> CancelOrderAsync(
        Guid id, CancelOrderRequest request, OrderService svc, HttpContext httpContext, CancellationToken ct)
    {
