...
export const DeleteOrderModal = ({ order, loading, onClose, onConfirm }: Props) => (
  <Modal title="Delete Order" onClose={onClose}>
    <div className="flex items-start gap-4">
      ...
      <div>
        <p className="font-semibold text-slate-900">Delete this order?</p>
        <p className="text-sm text-slate-500 mt-1.5">
          Order <span className="font-mono font-semibold text-slate-700">{order.orderNumber}</span> will be permanently deleted. This cannot be undone.
        </p>
      </div>
    </div>
